module.exports = {
	...
	add: (pool, user, cb)=>{
		pool.getConnection(function(err, connection){
			...
			else
			{
				...
				query = "INSERT INTO users(user_name, first_name, last_name, password, email, date_of_birth) VALUES('"+user.usrname+"', '"+user.firstName+"', '"+user.lastName+"', '"+user.password+"', '"+user.email+"', '"+user.dateOfBirth+"')";
	...
	updateStatus: (pool, user, cb)=>{
		pool.getConnection(function(err, connection){
			...
			{
				console.log('connected as id ' + connection.threadId);
				query = "UPDATE users SET status = '"+user.status+"', status_time = NOW() WHERE id = "+user.id;
				connection.query(query, function(err,rows){
					connection.release();
					if(err)
						cb(err, null);
